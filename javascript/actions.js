    ...
    function BackgroundMusicClass() {
        var musicSound=null;
          this.isnull=function() {
              var truthVal=(musicSound==null)?true:false;
              return truthVal;
          }
...
var gameOverSound=new BackgroundMusicClass();
...
var die = function(){
    ...
    gameOverSound=new BackgroundMusicClass();
    gameOverSound.play("assets/sounds/gameOver.mp3");
...
function playGame(){
    ...
    if(!died){
        if(clicked){
            if(flagged==0) {
                backgroundMusic = new BackgroundMusicClass();
                backgroundMusic.loopSong("assets/sounds/background.mp3");
                flagged=1;
            }
            p.spd = p.spdMax;
            if(p.acc == 0){
...
function pauseGame() {
    if(!paused) {
        if(!backgroundMusic.isPaused())
                backgroundMusic.startOrStopMusic();
        if(!gameOverSound.isended()) {
            if(died)
                gameOverSound.startOrStopMusic();
        }
        clearInterval(t);
        remaining=1000/FPS-Date.now()+startTime;
        modals("pause");
        paused=true;
    }
...
function play() {
    if(paused) {
        var t0=setTimeout(function () {
            paused=false;
            playGame();
            t=setInterval(playGame, 1000/FPS);
        }, remaining);
    } 
}
...
function modals(item) {
    ...
    if(item=="pause") {
        ...
        span.onclick = function() {
                    ...
                    if(!died)
                        backgroundMusic.startOrStopMusic();
                    else if(!gameOverSound.isended()) 
                        gameOverSound.startOrStopMusic();
                    if(backgroundMusic.isnull() && (!muted)) {
                        flagged=0;
                    }
                    play();
                    }
...
function addHtml(button) {
    ...
    if(button=="pause") {
        var muteText=muted?"UNMUTE": "MUTE SOUND";
        modalheading.innerHTML="OPTIONS";
        modalbody.innerHTML="<div class='pauseMenu'><button id='resume'>RESUME</button><button onclick=\"window.location.href='index.html'\">GO TO HOME</button><button id='mute'>"+muteText+"</button></div>";
    }
    console.log(modalbody);
    var resumeButton=modalbody.querySelector('#resume');
    var muteButton=modalbody.querySelector('#mute');
    resumeButton.addEventListener("click", function() {
        document.getElementsByClassName("close1")[0].click();
    });
    muteButton.addEventListener("click", function() {
        muted=muted?false:true;
        var muteUpdateText=(muted?"UNMUTE": "MUTE SOUND");
        this.innerHTML=muteUpdateText;
    });
}
});
document.addEventListener("keydown", function (e) {
    if(e.key=='p')
        document.getElementById("pausing").click();
});